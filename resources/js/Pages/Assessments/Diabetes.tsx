            ...
            <AppLayout>
                ...
                <div className="max-w-xl mx-auto">
                    <div className="bg-surface-container-lowest rounded-lg p-6 md:p-10 shadow-[0_30px_60px_-15px_rgba(2,103,131,0.06)] text-center">
                        ...
                        <p className="font-label-md text-on-surface-variant">Skor Risiko Diabetes Anda</p>
            ...
            <div className="max-w-3xl mx-auto">
                <div className="mb-16 text-center">
                    <div className="flex justify-between items-end mb-3">
                        <span className="font-label-md text-primary font-bold">Langkah {step + 1} dari {questions.length}</span>
                        <span className="font-label-md text-on-surface-variant">{Math.round(((step + 1) / questions.length) * 100)}% Selesai</span>
                    </div>
                ...
                <div className="bg-surface-container-lowest rounded-lg p-6 md:p-10 shadow-[0_30px_60px_-15px_rgba(2,103,131,0.06)]">
                    <div className="mb-10">
                        <span className="inline-block px-3 py-1 bg-tertiary-fixed text-on-tertiary-fixed-variant rounded-full font-label-sm uppercase tracking-wider mb-6">
                            {current.category}
                        </span>
                        <div className="flex items-start gap-6">
                            ...
                            <h2 className="font-headline-lg-mobile md:font-headline-lg text-[28px] md:text-[32px] text-on-surface leading-tight">
                                {current.text}
                            </h2>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 gap-6">
                        {current.options.map(([value, label]) => (
                            <button key={value} onClick={() => handleAnswer(value)}
                                className={`group flex items-center p-6 border-[1.5px] border-outline-variant rounded-lg bg-surface transition-all duration-300 hover:border-primary hover:bg-surface-container-low text-left ${
                                    answers[current.id] === value ? 'border-primary bg-primary/5' : ''
                                }`}>
                                <div className="flex-1">
                                    <p className="font-body-lg text-on-surface font-semibold">{label}</p>
                                </div>
